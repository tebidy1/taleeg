...
const prompt_builder_1 = require("./prompt_builder");
...
function extractEnglishPhrase(text) {
...
function stressSpan(phrase) {
    const words = phrase.replace(/[.,!?]/g, "").split(/\s+/).filter(Boolean);
    let best = "";
    for (const w of words) {
        if (!FUNCTION_WORDS.has(w.toLowerCase()) && w.length > best.length)
            best = w;
    }
...
class SessionOrchestrator {
    o;
    startTime = Date.now(); // effective start (shifts forward on pause)
    timer = null;
    phase = "flash_open";
    injectedPhases = new Set(["flash_open", "warmup", "quick_review"]); // in initial prompt
    lastRealtimeUpdateS = 0;
    wrapNudgeSent = false;
    ended = false;
    ...
    // live signals
    modelSpeaking = false;
    userSpokeSinceTurnComplete = false;
    ...
    // with a false "retry".
    static CORRECTION_MARKERS = ["تقصد", "بنطق", "ركّز", "ركز"];
    detectTurnFeedback(captainText) {
    ...
    tick() {
        ...
        // 2) phase transitions
        const newPhase = (0, prompt_builder_1.detectCurrentPhase)(elapsed);
        ...
        if (!this.wrapNudgeSent && elapsed >= WRAP_NUDGE_S && !this.modelSpeaking) {
            ...
            this.injectDirective("TIME IS UP. Move to Victory Close NOW: ask for the hero word, one-sentence celebration, one-sentence teaser for tomorrow, final production of today's sentence, then say goodbye warmly. Keep each turn short.", true);
        }
        ...
        if (this.lastTurnCompleteAt > 0 &&
            ...
            if (this.awaitingStudent) {
                ...
                this.injectDirective("You asked the student to speak but they've gone quiet. Gently encourage them or offer a tiny hint (the first word or two), then wait again. Do NOT skip ahead or answer for them.", true);
            }
            else {
                this.log({ type: "narration_beat" });
                console.log(`🎬 Narration beat (${NARRATION_BEAT_MS}ms) — resume story`);
                this.injectDirective("That pause was a dramatic beat, not a student cue — nothing was asked of the child. Continue your narration from exactly where you stopped. Do NOT re-greet and do NOT ask the student to repeat anything; just flow into the next line of the script.", true);
            }
        }
    ...
    onPhaseChange(phase) {
        ...
        if (phase === "end") {
            if (!this.wrapNudgeSent) {
                this.wrapNudgeSent = true;
                this.injectDirective("The session is over. Say a single short warm goodbye sentence now and stop.", !this.modelSpeaking);
            }
            return;
        }
    ...
    injectRealtimeUpdate(elapsed) {
        ...
        const update = this.o.promptBuilder.buildRealTimeUpdate({ student: this.o.student, mission: this.o.mission, sessionStartTime: new Date(this.startTime) }, state);
    ...
    estimateMood() {
        ...
        if (this.studentSentences >= 4 && silentFor < 20 && this.hintsUsed === 0)
            return "confident";
        return "neutral";
    }
    estimateEngagement(elapsed) {
        if (this.lastUserTranscriptAt === 0)
            return elapsed > 60 ? "low" : "high";
    ...
    injectDirective(text, active) {
        ...
        if (active && this.micOpen)
            this.closeMicActivity();
        try {
            this.o.geminiSession.sendClientContent({
    ...
    sendToClient(payload) {
        if (this.o.ws.readyState === ws_1.WebSocket.OPEN) {
            this.o.ws.send(JSON.stringify(payload));
        }
    }
    log(event) {
        (0, memory_1.logSessionEvent)(this.o.sessionId, event);
    }
    ...
    async finalize() {
        const transcript = (0, memory_1.readSessionTranscript)(this.o.sessionId);
        ...
        if (!parsed)
            return;
        (0, memory_1.saveSessionSummary)(this.o.student.id, {
