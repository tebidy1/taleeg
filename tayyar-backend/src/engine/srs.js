...
exports.reviewVocabularyItem = reviewVocabularyItem;
 ...
 */
function reviewVocabularyItem(item, performanceScore, context, responseType) {
    const now = new Date();
    const record = {
        date: now,
    ...
    if (performanceScore >= SRS_CONFIG.MASTERY_THRESHOLD) {
        ...
        newEaseFactor = Math.min(SRS_CONFIG.MAX_EASE_FACTOR, item.ease_factor + 0.1);
    }
    else if (performanceScore >= SRS_CONFIG.PASSING_THRESHOLD) {
        // OK response — slight decrease, but keep progressing
        newInterval = Math.max(SRS_CONFIG.MIN_INTERVAL, Math.round(item.interval_days * 0.8));
        newEaseFactor = Math.max(SRS_CONFIG.MIN_EASE_FACTOR, item.ease_factor - 0.15);
    }
    else {
        // FAILED response — reset to short interval
        newInterval = SRS_CONFIG.MIN_INTERVAL;
        newEaseFactor = Math.max(SRS_CONFIG.MIN_EASE_FACTOR, item.ease_factor - 0.3);
    }
    // Apply bounds
    ...
    const newMasteryLevel = calculateMasteryLevel(item.review_count + 1, item.mastery_level, performanceScore, responseType);
    // Update pronunciation/production accuracy
    const newPronunciationAccuracy = responseType === "recognition" || responseType === "recall"
        ? (item.pronunciation_accuracy * item.review_count + performanceScore) /
            (item.review_count + 1)
        : item.pronunciation_accuracy;
    const newProductionAccuracy = responseType === "production"
...
// ============================================================
function calculateMasteryLevel(totalReviews, currentLevel, lastScore, responseType) {
    ...
    if (totalReviews >= 5 && lastScore >= SRS_CONFIG.MASTERY_THRESHOLD) {
        return Math.max(currentLevel, (3 + productionBonus));
    }
    if (totalReviews >= 3 && lastScore >= SRS_CONFIG.MASTERY_THRESHOLD) {
        return Math.max(currentLevel, (2 + productionBonus));
    }
...
// ============================================================
function getDueReviews(vocabulary, currentDate = new Date()) {
    return vocabulary
        .filter((item) => {
        ...
        const isOverdue = item.last_reviewed &&
...
// ============================================================
function updatePatternMastery(mastery, stage, success, context) {
    const now = new Date();
    const stageMap = {
    ...
    if (success && currentLevel < 5) {
        updatedMastery[levelKey] = currentLevel + 1;
    }
...
// ============================================================
function getReviewsForMission(vocabulary, maxReviews = 3) {
    ...
    const learning = due.filter((item) => item.mastery_level === 1 ||
 ...
 */
function createNewVocabularyItem(word, patternId, phonemeFocus) {
    const now = new Date();
    const nextReview = new Date(now);
 ...
 */
function getStudentProgress(vocabulary) {
    ...
    const learning = vocabulary.filter((v) => v.mastery_level >= 1 && v.mastery_level < 4).length;
    const newWords = vocabulary.filter((v) => v.mastery_level === 0).length;
    const reviewedItems = vocabulary.filter((v) => v.review_count > 0);
    const avgAccuracy = reviewedItems.length > 0
