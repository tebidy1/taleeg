...
import { getStudentProgress } from "../engine/srs";
...
export function generateWeeklyReport(
  ...
  const newWords = weekSessions.reduce(
    (sum, s) => sum + s.new_vocabulary.length,
    0
  );
  ...
  // Progress metrics
  const progress = getStudentProgress(student.vocabulary);
...
export function generateCelebrationMessage(
  ...
  const messages: Record<string, string> = {
    ...
    level_1_complete:
      `🎓 مبروك! ${student.name} أكمل المستوى الأول!\n\nالإحصائيات:
...
function getMoodDescription(emoji: string): string {
...
function formatPronunciationSummary(session: Session): {
  average: number;
  issues: string[];
} {
  ...
  const avg =
    session.pronunciation_scores.reduce(
      (sum, p) => sum + p.overall_accuracy,
      0
    ) / session.pronunciation_scores.length;
...
function getPhonemeHint(phoneme: string): string {
...
function formatComparison(student: Student, session: Session): string {
...
function formatTomorrowPreview(currentMissionId: string): string {
...
function formatProgressBar(
  ...
  const changeStr =
    change > 0 ? ` (+${Math.round(change * 100)}%)` : change < 0 ? ` (${Math.round(change * 100)}%)` : "";
  return `   ${label}: ${Math.round(start * 100)}% → ${Math.round(end * 100)}%${changeStr}`;
}

function formatPronunciationTrend(sessions: Session[]): {
  ...
  const startAcc =
    firstSession.pronunciation_scores.reduce(
      (sum, p) => sum + p.overall_accuracy,
      0
    ) / Math.max(firstSession.pronunciation_scores.length, 1);
  const endAcc =
    lastSession.pronunciation_scores.reduce(
      (sum, p) => sum + p.overall_accuracy,
      0
    ) / Math.max(lastSession.pronunciation_scores.length, 1);
...
function findBestMoment(sessions: Session[]): string {
  // Find session with highest points or best pronunciation
  const best = sessions.reduce((best, s) =>
    s.total_points > best.total_points ? s : best
  , sessions[0]);
...
function generateTeacherNote(student: Student, sessions: Session[]): string | null {
...
function getDaysUntilRenewal(student: Student): number {
